...
| Lifecycle helpers | install, doctor, login check, healthcheck, anonymous SOS report, backup, restore, update, reset dry-run, restore-based rollback, no-lock-in Compose usage | Exact `v1.4.0` compatibility is validated for the component tuple listed in the compatibility matrix. |
