...
`v1.4.0` is the latest published release for the documented component tuple, but exact-tag/package-artifact validation is pending. `v1.3.1` remains the latest validated-compatible release and its published operator-bundle artifact passed the lifecycle validation matrix for the documented scope.
