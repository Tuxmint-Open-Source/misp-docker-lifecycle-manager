...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_release_docs_require_exact_tag_compatibility_validation(self):
        ...
        self.assertIn('Backup, restore, and rollback | 🟡 `v1.0.0` release-tag evidence exists', readiness)
        self.assertIn('Browser-facing login | 🟡 `v1.0.0` release-tag evidence exists', readiness)
        self.assertIn('No-lock-in Compose operation | 🟡 `v1.0.0` release-tag evidence exists', readiness)
        self.assertIn('`v1.1.0` release-validation state', readiness)
        self.assertIn('not called validated compatible until its immutable tag passes', readiness)
        ...
        self.assertIn('Exact component compatibility remains pending', deployment)
    ...
    def test_security_policy_exists_and_guides_private_reporting(self):
        ...
        self.assertIn('`v1.1.0` | Current stable release line', security)
        self.assertIn('`v1.0.0` | Previous stable release and latest validated-compatible artifact', security)
    ...
    def test_v1_1_release_is_pending_and_v1_0_evidence_is_preserved(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        ...
        self.assertIn('Stable release line', readme)
        ...
        self.assertIn('`v1.1.0` pending exact-tag validation', readiness)
        self.assertIn('`v1.0.0` remains validated compatible', readiness)
        self.assertIn('core `v2.5.44`, modules `v3.0.9`, guard `v1.2`', readiness)
        self.assertIn('| `v1.1.0` | Current stable release line', security)
        self.assertIn('| `v1.0.0` | Previous stable release', security)
