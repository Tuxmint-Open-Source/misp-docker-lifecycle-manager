...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_release_docs_require_exact_tag_compatibility_validation(self):
        ...
        self.assertIn('Backup, restore, and rollback | ✅ `v1.3.1` exact-tag and packaged-artifact evidence; ✅ `v1.3.0` exact-tag and packaged-artifact evidence', readiness)
        self.assertIn('Browser-facing login | ✅ `v1.3.1` exact-tag and packaged-artifact evidence; ✅ `v1.3.0` exact-tag browser evidence', readiness)
        self.assertIn('No-lock-in Compose operation | ✅ `v1.3.1` exact-tag and packaged-artifact evidence; ✅ `v1.3.0` exact-tag and packaged-artifact evidence', readiness)
    ...
    def test_community_health_files_exist_and_are_public_safe(self):
        ...
        self.assertIn('v1.3.1, v1.3.0, or commit SHA', bug_template)
    ...
    def test_security_policy_exists_and_guides_private_reporting(self):
        ...
        self.assertIn('`v1.3.1` | Current stable release line and latest validated-compatible artifact', security)
    ...
    def test_v1_3_release_is_pending_and_v1_2_evidence_is_preserved(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        ...
        self.assertIn('Current `VERSION` value on `main`: `1.3.1`', readme)
