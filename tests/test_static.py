...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_release_docs_require_exact_tag_compatibility_validation(self):
        ...
        self.assertIn('Backup, restore, and rollback | ✅ `v1.4.0` exact-tag and published-artifact evidence', readiness)
        self.assertIn('Browser-facing login | ✅ `v1.4.0` exact-tag and published-artifact evidence', readiness)
        self.assertIn('No-lock-in Compose operation | ✅ `v1.4.0` exact-tag and published-artifact evidence', readiness)
        ...
        self.assertIn('`v1.4.1` is the latest published documentation/hosted-docs patch release', deployment)
        self.assertIn('`v1.4.0` remains the latest validated-compatible release', deployment)
        ...
        self.assertIn('No-lock-in behavior passed exact-tag and published-artifact validation for `v1.4.0`', deployment)
    ...
    def test_documentation_red_line_entry_points_exist(self):
        ...
        self.assertIn('`v1.4.1` is the latest published release', docs_readme)
        self.assertIn('`v1.4.0` remains the latest validated-compatible release', docs_readme)
    ...
    def test_hosted_documentation_separates_current_and_archived_evidence(self):
        ...
        self.assertIn('`v1.4.1` exact-tag validation is pending', archive)
    ...
    def test_security_policy_exists_and_guides_private_reporting(self):
        ...
        self.assertIn('`v1.4.1` | Current published release; compatibility validation pending', security)
        self.assertIn('`v1.4.0` | Latest validated-compatible release', security)
    ...
    def test_dependency_and_code_scanning_automation_is_configured(self):
        ...
        self.assertIn('github/codeql-action/init@e4fba868fa4b1b91e1fdab776edc8cfbe6e9fb81', codeql)
        self.assertIn('github/codeql-action/analyze@e4fba868fa4b1b91e1fdab776edc8cfbe6e9fb81', codeql)
    ...
    def test_v1_4_release_is_pending_and_v1_3_1_evidence_is_preserved(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        compatibility = (ROOT / 'docs' / 'compatibility.md').read_text()
        matrix = (ROOT / 'docs' / 'validation' / 'matrix.md').read_text()
        ...
        self.assertIn('exact-tag compatibility validation pending', readme)
        ...
        self.assertIn('latest published release is `v1.4.1`', readme)
        self.assertIn('current validated tuple remains manager `v1.4.0`, MISP core `v2.5.44`, modules `v3.0.9`, and guard `v1.2`', readme)
        self.assertIn('docs/validation/README.md', readme)
        self.assertIn('| `v1.4.1` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | 🟡 Pending validation | Pending | — |', compatibility)
        ...
        self.assertIn('| `v1.4.1` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | 🟡 Pending validation | — | — | — | — | — | — |', matrix)
        ...
        self.assertEqual(channels['latest_validated'], 'v1.4.0')
