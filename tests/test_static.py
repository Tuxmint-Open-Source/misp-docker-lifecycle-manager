...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_remote_proxy_bind_and_firewall_ownership_are_documented(self):
        ...
        self.assertIn('--ctstate NEW', deployment)
    ...
    def test_upstream_monitoring_workflow_actions_are_sha_pinned(self):
        ...
        self.assertIn('uses: actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a', text)
        self.assertIn('uses: actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c', text)
    ...
    def test_operator_bundle_release_asset_workflow_is_guarded(self):
        workflow = (ROOT / '.github' / 'workflows' / 'operator-bundle-release-assets.yml').read_text()
        release_docs = (ROOT / 'docs' / 'release' / 'release-process.md').read_text()
        ...
        self.assertIn('default: HEAD', workflow)
        self.assertIn('allow_non_tag=true', workflow)
        ...
        self.assertIn('uses: actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1', workflow)
        self.assertIn('uses: actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a', workflow)
        self.assertIn('uses: actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c', workflow)
        self.assertNotIn('uses: actions/checkout@v', workflow)
        self.assertNotIn('uses: actions/upload-artifact@v', workflow)
        self.assertNotIn('uses: actions/download-artifact@v', workflow)
        self.assertIn('misp-docker-lifecycle-manager-${TAG}.tar.gz', workflow)
        self.assertIn('misp-docker-lifecycle-manager-${TAG}.tar.gz.sha256', workflow)
        self.assertIn('release assets require an immutable vX.Y.Z tag', workflow)
        self.assertIn('read-only job', release_docs)
        self.assertIn('release-write permissions only in the', release_docs)
        self.assertIn('workflow dry runs', bundle_docs)
        self.assertIn('For supported operator installs', bundle_docs)
        self.assertIn('checksummed operator bundle from the same GitHub Release', bundle_docs)

    def test_upstream_input_policy_separates_mutable_refs_from_validation_identity(self):
        policy = (ROOT / 'docs' / 'upstream-inputs.md').read_text()
        ...
        self.assertIn('Immutable upstream Git and container input policy', policy)
        self.assertIn('Discovery reference', policy)
        self.assertIn('Development reference', policy)
        self.assertIn('Release identity', policy)
        self.assertIn('Upstream Git identity', policy)
        self.assertIn('Component identity', policy)
        self.assertIn('Container image digest', policy)
        self.assertIn('Mutable upstream branches are allowed for discovery and normal update workflows', policy)
        self.assertIn('must not be presented as immutable compatibility evidence', policy)
        self.assertIn('Runtime `latest` image tracking is a convenience mode', policy)
        self.assertIn('The watcher must not create a compatibility claim', policy)
        self.assertIn('Adopting digest-pinned images requires a separate implementation decision', policy)
        self.assertIn('Until then, validated compatibility is expressed through official component tags plus the exact manager release/ref', policy)
    ...
    def test_release_integrity_policy_documents_current_controls_and_deferrals(self):
        ...
        self.assertIn('Release integrity and provenance policy', policy)
        self.assertIn('Companion SHA-256 files', policy)
        self.assertIn('Signed Git tags | Deferred', policy)
        self.assertIn('Signed release assets | Deferred', policy)
        self.assertIn('SBOMs | Deferred', policy)
        self.assertIn('Build provenance or attestations | Deferred', policy)
        self.assertIn('OCI image digests for upstream MISP containers | Deferred to the upstream-input policy', policy)
        self.assertIn('Deferred means not implemented and not claimed', policy)
        self.assertIn('Do not pipe downloaded release assets directly into a shell', policy)
        self.assertIn('sha256sum --check misp-docker-lifecycle-manager-vX.Y.Z.tar.gz.sha256', policy)
        self.assertIn('checksum', policy)
        self.assertIn('corruption', policy)
        ...
        self.assertIn('checksums are documented as corruption/mismatch detection', qa)
        self.assertIn('Document the release integrity and provenance policy', changelog)
    ...
    def test_release_docs_require_exact_tag_compatibility_validation(self):
        ...
        self.assertIn('Backup, restore, and rollback | ✅ `v1.4.1` exact-tag and published-artifact evidence', readiness)
        self.assertIn('Browser-facing login | ✅ `v1.4.1` exact-tag and published-artifact evidence', readiness)
        self.assertIn('No-lock-in Compose operation | ✅ `v1.4.1` exact-tag and published-artifact evidence', readiness)
        self.assertIn('`v1.4.1` release-validation state', readiness)
        self.assertIn('`v1.4.0` release-validation state', readiness)
        self.assertIn('`v1.3.1` release-validation state', readiness)
        self.assertIn('Latest published manager release | `v1.4.1`', readiness)
        self.assertNotIn('Next manager release', readiness)
        self.assertIn('`v1.3.0` release-validation state', readiness)
        self.assertIn('`v1.2.0` release-validation state', readiness)
        ...
        self.assertIn('healthy-path execution, `OK` mapping, and performance-data ingestion through Nagios XI', readiness)
        self.assertIn('Native non-OK transitions, recovery, notifications, and the other named monitoring platforms remain unvalidated', readiness)
        ...
        self.assertIn('documented stable release scope', deployment)
        ...
        self.assertIn('No-lock-in behavior passed exact-tag and published-artifact validation for `v1.4.1`', deployment)
        self.assertIn('The `--proxy-bind-address` option is available in `v1.4.0` and is not available in `v1.3.1`', deployment)
    ...
    def test_documentation_red_line_entry_points_exist(self):
        ...
        self.assertIn('Do not move or rewrite old release tags to make hosted pages build', docs_readme)
        self.assertIn('RTD `latest` remains the default hosted version until a post-foundation release tag is active, built, and verified as `stable`', docs_readme)
        self.assertIn('`v1.4.1` is the latest published and latest validated-compatible release', docs_readme)
        ...
        self.assertIn(f"git checkout {channels['latest_validated']}", getting_started)
    ...
    def test_hosted_documentation_separates_current_and_archived_evidence(self):
        ...
        self.assertIn('`v1.4.1` bundle passed', bundle)
    ...
    def test_monitoring_contract_is_documented_and_healthcheck_command_matches_it(self):
        monitoring = (ROOT / 'docs' / 'monitoring.md').read_text()
        nagios_setup = (ROOT / 'docs' / 'nagios-xi-ncpa.md').read_text()
        readme = (ROOT / 'README.md').read_text()
        docs_index = (ROOT / 'docs' / 'README.md').read_text()
        contributor_path = (ROOT / 'docs' / 'contribute-and-maintain.md').read_text()
        operator = (ROOT / 'docs' / 'operator-guide.md').read_text()
        ...
        self.assertIn('nagios-xi-ncpa.md', docs_index)
        self.assertIn('Nagios XI with NCPA: nagios-xi-ncpa.md', (ROOT / 'mkdocs.yml').read_text())
        ...
        self.assertIn('Nagios XI `2026R1.6.1`', monitoring)
        self.assertIn('NCPA `3.4.3-1`', monitoring)
        self.assertIn('Complete native status-transition and alerting validation', monitoring)
        self.assertIn('Nagios XI with NCPA setup guide', monitoring)
        self.assertIn('/usr/local/ncpa/plugins/check_misp_dlm', nagios_setup)
        self.assertIn('follow_symlinks = 0', nagios_setup)
        self.assertIn('plugin_timeout = 35', nagios_setup)
        self.assertIn('sudo visudo -cf /etc/sudoers.d/ncpa-misp-dlm', nagios_setup)
        self.assertIn('nagios ALL=(root) NOPASSWD:', nagios_setup)
        self.assertIn('sudo -u nagios /usr/local/ncpa/plugins/check_misp_dlm', nagios_setup)
        self.assertIn("-M 'plugins/check_misp_dlm'", nagios_setup)
        self.assertIn('Plugin Arguments | leave completely empty', nagios_setup)
        self.assertIn('Do not make the managed deployment', nagios_setup)
        self.assertIn('Do not disable SELinux', nagios_setup)
        self.assertNotIn('nagios ALL=(root) NOPASSWD: ALL', nagios_setup)
        ...
        nagios_report = (ROOT / 'docs' / 'validation' / 'nagios-xi-ncpa-v1.4.1.md').read_text()
        self.assertIn('MISP Docker Lifecycle Manager | `v1.4.1`', nagios_report)
        self.assertIn('Nagios XI | `2026R1.6.1`', nagios_report)
        self.assertIn('NCPA agent | `3.4.3-1` (`x86_64`)', nagios_report)
        self.assertIn('Plugin Arguments | empty', nagios_report)
        self.assertIn('did **not** deliberately exercise', nagios_report)
        self.assertIn('notification delivery or escalation', nagios_report)
    ...
    def test_community_health_files_exist_and_are_public_safe(self):
        ...
        self.assertIn('v1.4.0, v1.3.1, or commit SHA', bug_template)
    ...
    def test_security_policy_exists_and_guides_private_reporting(self):
        ...
        self.assertIn('`v1.3.0` | Previous stable release with retained compatibility evidence', security)
    ...
    def test_dependency_and_code_scanning_automation_is_configured(self):
        ...
        self.assertIn('github/codeql-action/init@cdf488f595d80d6e07e03d4674febd5ab45fa938', codeql)
        self.assertIn('github/codeql-action/analyze@cdf488f595d80d6e07e03d4674febd5ab45fa938', codeql)
        self.assertIn('github/codeql-action@v4', codeql)
        self.assertNotIn('@v3', codeql)

        dependabot = (ROOT / '.github' / 'dependabot.yml').read_text()
        self.assertIn('workflow-bootstrap-actions:', dependabot)
        self.assertIn('"actions/checkout"', dependabot)
        self.assertIn('"actions/setup-python"', dependabot)
        self.assertIn('artifact-actions:', dependabot)
        self.assertIn('"actions/upload-artifact"', dependabot)
        self.assertIn('"actions/download-artifact"', dependabot)
        self.assertIn('codeql-actions:', dependabot)
    ...
    def test_always_running_repository_gate_is_read_only_and_complete(self):
        ...
        self.assertIn('uses: actions/setup-python@5fda3b95a4ea91299a34e894583c3862153e4b97', workflow)
    ...
    def test_maintainer_workflow_documents_repo_operations(self):
        ...
        self.assertIn('Dependabot runs weekly and groups coupled GitHub Actions', maintainer)
        self.assertIn('Do not auto-merge GitHub Actions updates', maintainer)
        self.assertIn('expected maintenance work, not an automation outage or product regression', maintainer)
    ...
    def test_v1_4_1_release_is_validated_and_historical_evidence_is_preserved(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        compatibility = (ROOT / 'docs' / 'compatibility.md').read_text()
        matrix = (ROOT / 'docs' / 'validation' / 'matrix.md').read_text()
        ...
        self.assertIn('Current `VERSION` value on `main`: `1.4.1`', readme)
        self.assertIn('Latest published | `v1.4.1`', readme)
        self.assertIn('Latest validated | `v1.4.1`', readme)
        self.assertIn('exact-tag compatibility evidence', readme)
        ...
        self.assertIn('| `v1.3.0` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | ✅ Validated compatible | 2026-07-23 |', compatibility)
        self.assertIn('`v1.2.0` release tag', compatibility)
        self.assertIn('| `v1.2.0` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | ✅ Validated compatible | 2026-07-22 |', compatibility)
        self.assertIn('compatibility-v1.3.1-misp-core-v2.5.44.md', compatibility)
        self.assertIn('compatibility-v1.3.0-misp-core-v2.5.44.md', compatibility)
        self.assertIn('compatibility-v1.2.0-misp-core-v2.5.44.md', compatibility)
        ...
        self.assertIn('| `v1.3.1` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | ✅ Validated compatible | ✅ | ✅ | ✅ | ✅ | ✅ |', matrix)
        self.assertIn('| `v1.3.0` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | ✅ Validated compatible | ✅ | ✅ | ✅ | ✅ | ✅ |', matrix)
        self.assertIn('`v1.2.0` release tag', matrix)
        self.assertIn('| `v1.2.0` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | ✅ Validated compatible | ✅ | ✅ | ✅ | ✅ | ✅ |', matrix)
        self.assertIn('compatibility-v1.3.1-misp-core-v2.5.44.md', matrix)
        self.assertIn('compatibility-v1.3.0-misp-core-v2.5.44.md', matrix)
        self.assertIn('compatibility-v1.2.0-misp-core-v2.5.44.md', matrix)
        ...
        self.assertIn('`v1.3.1` release-validation state', readiness)
        self.assertIn('`v1.3.0` release-validation state', readiness)
        self.assertIn('`v1.3.1` packages lifecycle safety hardening added after the `v1.3.0` tag', readiness)
        ...
        self.assertIn('| `v1.1.0` | Previous stable release with retained compatibility evidence', security)
        self.assertIn('| `v1.0.0` | Previous stable release with retained historical compatibility evidence', security)
        self.assertIn('## [1.4.1] - 2026-07-31', changelog)
        ...
        self.assertIn('Overall result | ✅ Validated compatible', v1_3_1_report)
        self.assertIn('published operator-bundle artifact', v1_3_1_report)
        self.assertIn('Monitoring healthcheck | ✅ Passed', v1_3_1_report)
        ...
        self.assertIn('Docker-aware filtering allowed the proxy and denied a separate source', v1_4_report)
        self.assertIn('Overall result | ✅ Validated compatible', v1_3_report)
        self.assertIn('Total duration | 3025 seconds', v1_3_report)
        self.assertIn('published operator-bundle artifact', v1_3_report)
        self.assertIn('canonical `./lifecycle/*.sh` command path', v1_3_report)
        self.assertIn('retained `./installer/*.sh` compatibility wrappers', v1_3_report)
        self.assertIn('Monitoring healthcheck | ✅ Passed', v1_3_report)
        ...
        self.assertEqual(channels['latest_published'], 'v1.4.1')
        self.assertEqual(channels['latest_validated'], 'v1.4.1')
