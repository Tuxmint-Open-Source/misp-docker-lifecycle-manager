...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_release_docs_require_exact_tag_compatibility_validation(self):
        ...
        self.assertIn('Backup, restore, and rollback | 🟡 `v1.3.0` pending; ✅ `v1.2.0` exact-tag and packaged-artifact evidence retained', readiness)
        self.assertIn('Browser-facing login | 🟡 `v1.3.0` pending; ✅ `v1.2.0` exact-tag browser evidence retained', readiness)
        self.assertIn('No-lock-in Compose operation | 🟡 `v1.3.0` pending; ✅ `v1.2.0` exact-tag and packaged-artifact evidence retained', readiness)
        ...
        self.assertIn('`v1.3.0` no-lock-in validation is pending', deployment)
        self.assertIn('No-lock-in behavior passed exact-tag and packaged-artifact validation for `v1.2.0`', deployment)
    ...
    def test_community_health_files_exist_and_are_public_safe(self):
        ...
        self.assertIn('v1.3.0, v1.2.0, or commit SHA', bug_template)
    ...
    def test_security_policy_exists_and_guides_private_reporting(self):
        ...
        self.assertIn('`v1.3.0` | Current stable release line; compatibility validation pending', security)
        self.assertIn('`v1.2.0` | Latest validated-compatible artifact and previous stable release evidence', security)
    ...
    def test_v1_3_release_is_pending_and_v1_2_evidence_is_preserved(self):
        ...
        self.assertIn('Current `VERSION` value on `main`: `1.3.0`', readme)
        self.assertIn('`v1.3.0` release tag', readme)
        self.assertIn('⏳ Pending exact-tag validation', readme)
        ...
        self.assertIn('git checkout v1.3.0', readme)
        self.assertIn('`v1.0.0` release tag', readme)
        self.assertIn('✅ Validated compatible', readme)
        self.assertIn('| `v1.3.0` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | 🟡 Pending validation | — | — |', compatibility)
        ...
        self.assertIn('| `v1.3.0` release tag | `v2.5.44` | `v3.0.9` | `v1.2` | 🟡 Pending validation | — | — | — | — | — | — |', matrix)
        ...
        self.assertIn('`v1.3.0` release-validation state', readiness)
        self.assertIn('`v1.2.0` remains the latest validated-compatible release', readiness)
        ...
        self.assertEqual(channels['latest_published'], 'v1.3.0')
        self.assertEqual(channels['latest_validated'], 'v1.2.0')
