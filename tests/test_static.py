...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_release_docs_require_exact_tag_compatibility_validation(self):
        ...
        self.assertIn('✅ validated for `v1.0.0-rc.3`', readiness)
        self.assertIn('Current-release browser login validation', readiness)
        self.assertIn('Public production-readiness validation report | ✅ for `v1.0.0-rc.3`', readiness)
    ...
    def test_documentation_red_line_entry_points_exist(self):
        ...
        self.assertIn('git checkout v1.0.0', getting_started)
    ...
    def test_maintainer_workflow_documents_repo_operations(self):
        ...
        self.assertIn('| B | `docker-compose.yml` service names', maintainer)
        self.assertIn('| C | `template.env`, README versioning guidance', maintainer)
    ...
    def test_final_v1_release_is_validated_compatible(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        ...
        self.assertIn('git checkout v1.0.0', readme)
