...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_release_docs_require_exact_tag_compatibility_validation(self):
        ...
        self.assertIn('Backup, restore, and rollback | ✅ `v1.2.0` exact-tag and packaged-artifact evidence', readiness)
        self.assertIn('Browser-facing login | ✅ `v1.2.0` exact-tag browser evidence', readiness)
        self.assertIn('No-lock-in Compose operation | ✅ `v1.2.0` exact-tag and packaged-artifact evidence', readiness)
    ...
    def test_security_policy_exists_and_guides_private_reporting(self):
        ...
        self.assertIn('`v1.2.0` | Current stable release line and latest validated-compatible artifact', security)
    ...
    def test_v1_2_release_is_pending_and_v1_1_evidence_is_preserved(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        ...
        self.assertIn('Current `VERSION` value on `main`: `1.2.0`', readme)
        ...
        self.assertIn('git checkout v1.2.0', readme)
        ...
        self.assertIn('`v1.2.0` validated compatible', readiness)
        ...
        self.assertEqual(channels['latest_published'], 'v1.2.0')
